...
def llr(clean_speech, processed_speech, fs, frameLen=0.03, overlap=0.75):
    ...
    frac=numerators/(denominators+1e-8)
    frac[frac<=0]=1000
...
def pesq_(clean_speech, processed_speech, fs):
    try:
...
def composite(clean_speech, processed_speech, fs):
    ...
    try:
        # pesq_mos = pesq_(clean_speech, processed_speech, fs)
        pesq_mos = pesq2(clean_speech, processed_speech)
    except Exception:
        return 0,0,0,0,0
        # pesq_mos = 0.0
...
def compareone(args):
    ...
    name = clean.split('/')[-1]
    # c = normalized_test_object(c)
    # p = normalized_test_object(p)
    ...
    try:
      ...
      ssnr,pesq,csig,cbak,covl = 0,0,0,0,0
...
def compare(refdir, degdir, use_tqdm=True):
    ...
    if use_tqdm:
      res = list(tqdm.tqdm(pool.imap(compareone, args), "Calculating", ncols=60))
    else:
      res = list(pool.imap(compareone, args))
    pool.close()
    pool.join()
    return res
